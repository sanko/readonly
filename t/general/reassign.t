...
my $err = qr/^Modification of a read-only value attempted/;
...
like $@ => qr[Attempt to delete readonly key '.+' from a restricted hash], 'Readonly::Hash reassign die';
...
SKIP:
{
	...
	# Reassign array
	eval q{Readonly @a2 => "something", "bad", "!"};
	like $@ => qr[Too many arguments for Readonly::Readonly], 'Readonly Array reassign die';
	ok eq_array(\@a2, [qw[another array value]]) => 'Readonly Array reassign no effect';

	# Reassign hash
	eval q{Readonly %h2 => {another => "bad", hash => "!"}};
	like $@ => qr[Attempt to delete], 'Readonly Hash reassign die';
	ok eq_hash(\%h2, {another => 'hash', of => 'things'}) => 'Readonly Hash reassign no effect';
};
